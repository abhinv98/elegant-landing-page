...
const Footer: React.FC = () => {
  ...
  const footerLinks = [
    {
      title: 'Product',
      links: ['Features', 'Integrations', 'Pricing', 'FAQ', 'Download']
    },
    {
      title: 'Company',
      links: ['About', 'Team', 'Contact', 'Careers', 'Press']
    },
    {
      title: 'Resources',
  ...
  return (
    <footer className={`py-12 ${theme === 'dark' ? 'bg-secondary-900 text-white' : 'bg-white text-secondary-900'}`}>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-2 md:grid-cols-5 gap-8">
          <div className="col-span-2">
            ...
            <p className={`mb-4 ${theme === 'dark' ? 'text-secondary-300' : 'text-secondary-600'}`}>
              Empowering your workflow with cutting-edge collaboration tools.
            </p>
            <div className="flex space-x-4">
              {socialIcons.map((icon, index) => (
                <motion.a
                  key={icon.name}
                  href="#"
                  className={`${theme === 'dark' ? 'text-secondary-300 hover:text-white' : 'text-secondary-600 hover:text-secondary-900'}`}
              ...
              <ul className="space-y-2">
                {column.links.map((link, linkIndex) => (
                  <li key={linkIndex}>
                    <motion.a
                      href="#"
                      className={`text-sm ${theme === 'dark' ? 'text-secondary-300 hover:text-white' : 'text-secondary-600 hover:text-secondary-900'}`}
                      whileHover={{ x: 5 }}
                      whileTap={{ scale: 0.95 }}
                    >
                      {link}
